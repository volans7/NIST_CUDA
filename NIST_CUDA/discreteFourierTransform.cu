 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
//void  __ogg_fdrffti(int n, double *wsave, int *ifac); // описано в dfft.c
...
DiscreteFourierTransform(int n)
{
	...
	printf("\nГармоники:");
	for (i = 0; i < n / 2; i++) // гармоники симметричны, поэтому считаем половину
	{
		m[i+1] = sqrt(pow(X[2 * i + 1], 2) + pow(X[2 * i + 2], 2)); 
		printf("%d) %lf ", i+1, m[i+1]);
	}
	...
	p_value = cephes_erfc(fabs(d)/sqrt(2.0));

	printf("\nРезультаты теста:\n");
	printf("count = %lf percentile = %lf N_l = %lf N_o = %lf d = %lf p_value = %lf", count, percentile, N_l, N_o, d, p_value);
